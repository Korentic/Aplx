...
# Ensure `aplx_llm.py` in the same folder can be imported from this script.
...
def load_aplx_checkpoint(
    ...
    if aplx_load_model is None:
        raise ImportError("Could not import aplx_llm.py from the same folder. Ensure it exists next to aplx_1.5.py.")
...
def ensure_aplx_native_engine(path: str = "aplx_checkpoints/best", device: str = "cpu"):
    ...
    if helpers is None:
        return False, "Could not import aplx_test.py. Place it next to aplx_1.5.py."
...
def print_aplx_red_interface():
    ...
    print(banner)
    print(f"{RED} [Aplx V1.5 is active - running locally - No Internet Required]{RESET}")
    print(f"{RED} [Ollama: {ollama_status} | Server: {server_status} | Model: {current_model} | Auto: {auto}]{RESET}")
    print(f"{RED} [Credits: R3nz, VS Code Copilot, Ollama, Gemini, Claude, minimax-m3 (MAJOR CODING DONE BY minimax-m3 and Visual Studios Copilot)]{RESET}")
    print(f"{RED} [Active Project: {active_project} | Persona: {persona} | Streaming: {SELF_STATE.get('streaming_enabled', False)}]{RESET}")
    print(f"{RED} [Welcome to the unreleased (in development) version of aplx, V1.5]{RESET}")
    ...
    print(f"{RED}/stream           {DIM}Toggle streaming output [NEW]{RESET}")
    print(f"{RED}/project          {DIM}Manage code project workspaces [NEW]{RESET}")
    print(f"{RED}/save             {DIM}Save last generated code [NEW]{RESET}")
    print(f"{RED}/git              {DIM}Git commit last saved files [NEW]{RESET}")
    print(f"{RED}/run              {DIM}Run last generated Python code [NEW]{RESET}")
    print(f"{RED}/stats            {DIM}Show productivity statistics [NEW]{RESET}")
    print(f"{RED}/persona          {DIM}Switch persona [NEW]{RESET}")
    print(f"{DIM}-------------------------------------------------------------{RESET}\n")
...
def improve_code(query: str) -> str:
    ...
    user_input = safe_input(f"{USER_NAME} (code):- ").strip()
    if user_input.lower() == 'last':
        ...
        while True:
            line = safe_input(f"{USER_NAME} (code):- ")
            if line.strip() == 'DONE':
                break
...
def greet_me():
    ...
    print(f"--- {greet}, {RED}Aplx AI is online..---")
...
def run_aplx_loop():
    while True:
        ...
        if query_lower.strip() in ['/model', 'model', 'switch model', 'change model']:
            ...
            speak("Type 'aplx' (for Aplx AI BETA), 'ollama', 'online', 'fast', 'smart', 'genius', 'code', or specify a model name:")
            ...
            elif model_choice.lower() == 'online':
                SELF_STATE['current_model_source'] = 'online'
                speak(APLX_PREFIX + "Online mode selected. Type the Ollama cloud model name you want to use (for example: minimax-m3):")
                online_model = safe_input(f"{RED}{user} (cloud model):-{RESET} ").strip()
                if online_model:
                    SELF_STATE['current_model'] = online_model
                    SELF_STATE['online_model'] = online_model
                    speak(APLX_PREFIX + f"Switched to online model: {online_model}")
                else:
                    SELF_STATE['current_model'] = SELF_STATE.get('smart_model', DEFAULT_MODEL_SMART)
                    speak(APLX_PREFIX + "No online model entered. Keeping the previous model.")
        ...
        if "exit" in query_lower or "sleep" in query_lower or "quit" in query_lower:
            ...
            while True:
                ...
                if SELF_STATE.get('streaming_enabled') and SELF_STATE.get('current_model_source', 'ollama') != 'aplx':
                    pass
                else:
            ...
            speak(APLX_PREFIX + "You are running the latest version of Aplx AI V2.0 NUCLEAR.")
...
def study_mode():
    print("\n--- ENTERING STUDY MODE ---")
    print("Aplx AI :- What notes would you like to view/write today?")
    note_name = safe_input(f"{USER_NAME} :- ").strip()
    if not note_name:
        print("Aplx AI :- Note name cannot be empty. Exiting Study Mode.")
        return
    ...
    if os.path.exists(file_path):
        ...
        try:
            ...
            pass
        print("-----------------------------")
        choice = safe_input("Aplx AI :- Edit? (yes/no): ").strip().lower()
    ...
    captured_lines = []
    while True:
        user_input = safe_input("{User} (notes) :- ")
...
def generate_response(user_input, timeout: int = 120):
    ...
    try:
        result = subprocess.run(
            [ollama_exec, "run", best_model, user_input],
            capture_output=True,
            text=True,
            encoding='utf-8',
            errors='ignore',
...
def _run_file_io():
    ...
    model   = kv.get("MODEL", "llama3.2:3b")
...
if __name__ == "__main__":
    ...
    if any(a.startswith('--input') or a.startswith('--output') for a in argv_tail):
        ...
        # your existing argparse CLI entry
        parser = argparse.ArgumentParser(description="Aplx AI query interface.")
        parser.add_argument('--query', type=str)
        parser.add_argument('--mode', type=str, choices=['chat', 'code'], default='chat')
        args = parser.parse_args()
        if args.query:
            print(run_query(args.query, mode=args.mode))
        else:
            main()  # your existing interactive loop
