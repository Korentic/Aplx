...
Approx. 1 Billion Parameters (1.004B) | 500k Token Context Window
================================================================================
...
-------------------------------------
  ...
  n_layers       = 17
  n_heads        = 16
  n_kv_heads     = 16  (can be reduced for GQA)
  hidden_dim     = 5,632  (SwiGLU intermediate, multiple of 256)
...
Total Parameters: ~1,004,607,488 (1.004B)
"""
...
def get_default_device(prefer_gpu: bool = True) -> torch.device:
    """Select a usable device across CPU, CUDA, and Apple Silicon."""
...
class ModelConfig:
    ...
    n_layers: int = 17
    n_heads: int = 16
    n_kv_heads: int = 16          # Set < n_heads for Grouped Query Attention
...
def run_basic_training_demo():
    ...
    demo_config = ModelConfig(
        ...
        training_seq_len=256,    # Training chunk size for demo (real: 4,096)
        sliding_window_size=256, # Sliding window for demo (real: 4,096)
    ...
    train_config = TrainingConfig(
        ...
        total_steps=50,              # Total training steps (real: 100K+)
        ...
        # Mixed Precision
        use_amp=(device.type == "cuda"),  # FP16 on GPU, FP32 on CPU
        amp_dtype="float16",
        
        # Logging & Checkpointing
        log_interval=5,              # Log every 5 steps
        eval_interval=25,            # Evaluate every 25 steps
        save_interval=25,            # Save checkpoint every 25 steps
